...
public class AudioPlay {
    ...
    private Handler timerHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case PLAYING:
                    if (playStatusListener != null && mPlayer != null) {
                        Log.i(TAG, "当前的播放进度/总进度：" + mPlayer.getCurrentPosition() + "  " + mPlayer.getDuration());
                        playStatusListener.onPlayDuration(((float)mPlayer.getCurrentPosition()) / ((float)mPlayer.getDuration()), calculateMusicLength(mPlayer.getCurrentPosition()));
                    }
                    break;
            }
        }
